...
function P2P_start() {
    ...
    p2p_server.fileInfo = {"Client-1" : {filePercentage : 0.2, filePosition : 0.0}, 
"Client-2" : {filePercentage : 0.4, filePosition : 0.2},
"Client-3" : {filePercentage : 0.1, filePosition : 0.6},
"Client-4" : {filePercentage : 0.1, filePosition : 0.7},
"Client-5" : {filePercentage : 0.2, filePosition : 0.8}};
...
function p2p_updateAnimation() {
    ...
    if(isRunning){
        ...
        for (var j = 0; j < p2p_clients.length; j++) {
            ...
            for (var l = 0; l < client.packetBuffer.length; l++){
                for (var c = 0; c < p2p_clients.length; c++){
                    if(client.type != p2p_clients[c].type)
                        p2p_travellingPackets.push(client.sendPacket(p2p_clients[c].type));
                }
            }
